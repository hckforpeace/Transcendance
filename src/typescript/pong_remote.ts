 ...
 * Define drawing var
 */
const	GOLDEN_NUMBER_REMOTE: number = 1.618033;
const	PLAYER_WIDTH_RATIO_REMOTE: number = 0.09;
const	PLAYER_WIDTH_HEIGHT_RATIO_REMOTE: number = 10;
var		PLAYER_HEIGHT: number;
var		PLAYER_WIDTH: number;
const	PLAYER_COLOR_REMOTE: string = "#FFFFFF";
const	PLAYER_SPEED_REMOTE: number = 7;

/* Score + Text */
const	FONT_NAME_REMOTE: string = "sans-serif";
var		FONT_SIZE: number;
const	SCORE_POS_RATIO_REMOTE: number = 0.9;
const	MSG_POS_RATIO_REMOTE: number = 0.7;

/* BallRemote */
const	BALL_INIT_SPEED_REMOTE: number = 8;
const	BALL_COLOR: string = "#FFFFFF";
const	BALL_MAX_SPEED_REMOTE: number = 50;
var		BALL_RAD2US: number;
/* Terrain draw */
const	TERRAIN_COLOR_REMOTE: string = "#FFFFFF";
var		TERRAIN_LINE_FAT: number;

/* Game status variable */
const	PLAYER_ONE_REMOTE: number = 1;
const	PLAYER_TWO_REMOTE: number = 2;
var		game_remote: PongRemote;
var		end_game_remote: boolean;
var		game_interval_remote: NodeJS.Timeout | number;
var		round_winner_remote: number;
var		p1_upPressed_remote: boolean = false;
var		p1_downPressed_remote: boolean = false;
var		p2_upPressed_remote: boolean = false;
var		p2_downPressed_remote: boolean = false;
...
class PongRemote {
	player_1: PlayerRemote;
	player_2: PlayerRemote;
	ball: BallRemote;
	score_max: number;
	new_round: boolean;
	...
	constructor(player_1_name: string, player_2_name: string, center: Vec2) {
		...
		this.player_1 = new PlayerRemote(player_1_name,
...
function draw_score_remote() {
	...
	let score_pos: Vec2 = { x: SCORE_POS_RATIO_REMOTE * canvas.width / 2,
 ...
 */
function draw_player_remote(player: PlayerRemote) {
	if (!canvas)
		throw new Error("Canvas not found");
	if (!ctx)
		throw new Error("Context not found");
 ...
 */
function draw_ball_remote(ball: BallRemote) {
	if (!ctx)
		throw new Error("Context not found.");
	ctx.beginPath();
...
function draw_finish_remote() {
	...
	if (!ctx)
		throw new Error("Context not found");
	let msg_pos: Vec2 = { x: MSG_POS_RATIO_REMOTE * canvas.width / 2,
 ...
 */
function pressedKeyHandler_remote(e: KeyboardEvent) {
 ...
 */
function releasedKeyHandler_remote(e: KeyboardEvent) {
	if (e.key === "Up" || e.key === "ArrowUp") {
    if (playerSide == 'p1')
 ...
 */
function launch_game_remote(p1_name: string, p2_name: string) {
	if (!canvas)
		throw new Error("Canvas not found");
	if (!p1_name || !p2_name)
 ...
 */
async function load_script_remote() {
  try {
